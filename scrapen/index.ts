...
const scrapeSupermarketPage = async ({ supermarketLabel, url, selectors }) => {
  ...
  const allProducts = await page.$$eval(selectors.product, (products, object) => {
    ...
    return products.map((product) => {
      ...
      if (supermarketLabel === 'hofer') {
        const possibleQuantityFields = product.querySelectorAll(supermarketSelector.quantity);
        possibleQuantityFields.forEach((field) => {
          if (field.innerText.includes('per')) singleProduct.quantity = field.innerText.trim();
        });
      } else {
